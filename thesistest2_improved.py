...
GPT_MODEL_NAME = "openai/gpt-5.2"  # OpenRouter model name (e.g., "openai/gpt-4o", "anthropic/claude-3.5-sonnet", etc.)
...
def ensure_dense_model_saved(local_path: Path, model_name: str):
    """Ensure the embedding model is saved locally. Improved version."""
    ...
    # Verify both config files exist (more reliable check)
...
@st.cache_resource
def get_gpt_client():
    """Get OpenRouter client instance with proper error handling."""
    ...
    try:
        client = openai.OpenAI(
            ...
            default_headers={ # Optional: Update with your app URL
                "X-Title": "Fitness Chatbot"  # Optional: Your app name
            }
        )
        return client
    except Exception as e:
        st.warning(f"⚠️ OpenRouter API Error: {e}")
        return None
...
def chat_with_bmi(question, weight, height, gender, age, chat_history, return_retrieval_info=False):
    ...
    # Always retrieve with details if we need to return them, otherwise just contexts
    ...
    if gpt_client:
        try:
            ...
            st.warning(f"OpenRouter API Error: {e}")
    ...
    fallback_exercises = pick_exercises_for_part("full body", bmi_val)
    fallback_text = "⚠️ AI model unavailable. Here are some exercises:\n\n" + "\n".join(fallback_exercises)
    if return_retrieval_info:
        return fallback_text, ex_titles, ex_distances
    return fallback_text
...
with st.sidebar:
    ...
    # Update current user if changed (using on_change callback would be better, but this works)
    ...
    # Use keys to prevent unnecessary reruns
